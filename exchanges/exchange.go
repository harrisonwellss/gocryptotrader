...
func (e *Base) SetCurrencyPairFormat() {
	...
	if e.Features.Supports.Trading.Spot {
		//if e.Config.CurrencyPairs.Pairs
	...
	/*
		...
		if e.CurrencyPairs.UseGlobalPairFormat {
			...
			if e.Config.CurrencyPairs.Pairs.Get(assets.AssetTypeSpot) == nil {
				e.Config.CurrencyPairs.Pairs.Store(assets.AssetTypeSpot,
...
func (e *Base) GetEnabledPairs(assetType assets.AssetType) currency.Pairs {
	...
	pairs, err := e.CurrencyPairs.GetPairs(assetType, true)
	if err != nil {
		log.Warnf("%s asset type %v enabled pairs err: %s", e.Name, assetType, err)
		return nil
	}
...
func (e *Base) GetAvailablePairs(assetType assets.AssetType) currency.Pairs {
	...
	if err != nil {
		log.Warnf("%s asset type %v available pairs err: %s", e.Name, assetType, err)
		return nil
	}
	return pairs.Format(format.Delimiter, format.Index, format.Uppercase)
}
...
func (e *Base) SetupDefaults(exch *config.ExchangeConfig) error {
	...
	if e.Features.Supports.Trading.Spot {
		p := exch.CurrencyPairs.Get(assets.AssetTypeSpot)
		if p != nil {
			e.CurrencyPairs.Store(assets.AssetTypeSpot, *p)
		} else {
			log.Warnf("AssetTypeSpot is nil")
		}
	}

	if e.Features.Supports.Trading.Futures {
		p := exch.CurrencyPairs.Get(assets.AssetTypeFutures)
		if p != nil {
			e.CurrencyPairs.Store(assets.AssetTypeFutures, *p)
		} else {
			log.Warnf("AssetTypeFutures is nil")
		}
	}
...
func (e *Base) UpdatePairs(exchangeProducts currency.Pairs, assetType assets.AssetType, enabled, force bool) error {
	...
	if err != nil {
		log.Warnf("%s UpdatePairs %s %v pairs is empty", e.Name, assetType, enabled)
	}
